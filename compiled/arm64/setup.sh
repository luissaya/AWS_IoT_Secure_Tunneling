...
if [ "$BUILD_CONFIG" = "y" ]; then
  while [ "$CONFIGURED" != 1 ]; do
    ...
    read -r ENDPOINT
    printf ${PMPT} "Specify full path to public PEM certificate:(Enter for deafult location: ./identity/*.pem.crt)"
    read -r CERT
    if [ "$CERT" ]; then
      CERT_DIR=$CERT
    fi
    printf ${PMPT} "Specify full path to private key:(Enter for deafult location: ./identity/*.pem.key)"
    read -r PRIVATE_KEY
    if [ "$PRIVATE_KEY" ]; then
